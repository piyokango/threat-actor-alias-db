...
def fetch_json(url: str) -> dict:
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "threat-actor-alias-db/0.1 (+https://github.com/piyokango/threat-actor-alias-db)"
        },
    )
    try:
    ...
    try:
        ...
        raise RuntimeError(f"fetched content is not valid JSON: {url}") from exc
